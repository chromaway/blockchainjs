...
function verifyHeader (currentHash, currentHeader, prevHash, prevHeader, target, isTestnet) {
  try {
    // check prevHash
    assert.equal(prevHash, currentHeader.prevblockid)
...
function Verified (connector, opts) {
  ...
  var syncBlockchain = util.makeSerial(function (newHash, newHeight) {
    ...
    if (!isReady || newHash === self.latest.blockid) {
      return
    }
  ...
  function onConnect () {
    self.connector.getHeader('latest')
      .then(function (header) {
        return syncBlockchain(header.blockid, header.height)
      })
      .catch(function (err) { self.emit('error', err) })
  }
  ...
  .then(function () {
    // set isReady is true and start syncing after initialization if connector connected
    isReady = true
...
Verified.prototype._initialize = function (opts) {
  ...
  .spread(function (chunkHashesCount, headersCount) {
    ...
    if (loadPreSaved) {
      return Promise.all([
        self.storage.setLastHash(opts.chunkHashes.lastBlockId),
  ...
  .spread(function (lastHash, chunkHashesCount, headersCount) {
    ...
    if (chunkHashesCount !== 0 || headersCount !== 0) {
      self.latest = {blockid: lastHash, height: headersCount}
      if (self.storage.compactMode) {
        self.latest.height += chunkHashesCount * 2016
      }

      timers.setImmediate(function () {
        self.emit('newBlock', self.latest.blockid, self.latest.height)
      })
    }
  })
}
...
Verified.prototype._sync = function (networkHash, networkHeight) {
  ...
  function syncThroughChunks (prevChunk) {
    ...
    function cachedGetHeader (id) {
      ...
      header.height = id
      header.blockid = util.hashEncode(util.sha256x2(rawHeader))
      return Promise.resolve(header)
    }

    // get not verified chunk for index
    self.connector.getHeader(index * 2016)
      .then(function (header) {
        return self.connector.getHeaders(header.blockid, {count: 2016})
      })
      .then(function (res) {
        var chunkHex = res.headers
        var prevHeaderPromise = index === 0
          ? {blockid: util.zfill('', 64)} // special case for zero header
      ...
      .spread(function (prevHeader, rawChunk) {
        ...
        if (firstHeader.prevblockid !== prevHeader.blockid) {
          index -= 1
          return timers.setImmediate(syncThroughChunks)
        }

        // remember for verifyHeader
        var prevHeaderHash = prevHeader.blockid
  ...
  function syncThroughHeaders (prevHeader) {
    ...
    return Promise.try(function () {
      ...
      return Promise.map(headersChain, function (header) {
        var chunkIndex = Math.floor(header.height / 2016)
        return getCachedTarget(chunkIndex, headersChain, getHeaderFn)
          .then(function (target) {
            verifyHeader(header.blockid, header,
                         prevHeader.blockid, prevHeader, target, self._isTestnet)
    ...
    .then(function () {
      return self.storage.setLastHash(_.last(headersChain).blockid)
    })
    .then(function () {
    ...
    .then(function () {
      ...
      self.latest = {blockid: latest.blockid, height: latest.height}
      self.emit('newBlock', self.latest.blockid, self.latest.height)
    })
  }
  ...
  if (delta <= 0 || delta > 50) {
    ...
    var heights = _.range(self.latest.height + 1, networkHeight + 1)
    var headers = heights.map(function (height) {
      return self.connector.getHeader(height)
    })
    ...
    .spread(function (chain, prevHeader) {
      ...
      if (chain[0].prevblockid !== prevHeader.blockid) {
...
Verified.prototype.getHeader = function (id) {
      ...
      .then(function (header) {
        return self.getHeader(header.height)
          .then(function (localHeader) {
            if (localHeader.blockid === header.blockid) {
              return localHeader
            }
  ...
  return Promise.try(function () {
    ...
    return Promise.try(function () {
      if (!self._chunkCache.has(headerChunkIndex) || self._chunkCache.get(headerChunkIndex).isRejected()) {
        var promise = self.connector.getHeader(headerChunkIndex * 2016)
          .then(function (header) {
            return Promise.all([
              self.connector.getHeaders(header.blockid),
 ...
 */
Verified.prototype.getTxBlockId = function (txId) {
  var self = this
  return self.connector.getTxBlockId(txId)
    .then(function (obj) {
      ...
      return self.getHeader(obj.data.blockid)
        .then(function (header) {
          ...
          if (header.merkleroot !== util.hashEncode(merkleHash)) {
