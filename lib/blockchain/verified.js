...
function Verified (connector, opts) {
  ...
  var syncBlockchain = util.makeCuncurrent(function (newHash, newHeight) {
  ...
  // wait when the storage will be ready and run initializea
...
Verified.prototype._syncStart = function () {
  if (!this.isSyncing()) this.emit('syncStart')
}

Verified.prototype._syncStop = function () {
...
Verified.prototype._sync = function (networkHash, networkHeight) {
  ...
  if (delta <= 0 || delta > 50) {
    ...
    var headers = self.connector.headersQuery(
      self.latest.height + 1, {to: networkHeight + 1})
