  ...
  function syncThroughChunks (prevChunk) {
    ...
    self.network.getHeader(index * 2016)
      ...
      .then(function (chunkHex) {
        ...
        if (index < networkIndex && chunkHex.length !== 160 * 2016) {
          throw new errors.Blockchain.VerifyChunkError(index, 'wrong length')
        }
        ...
        return Promise.all([new Buffer(chunkHex, 'hex'), prevHeaderPromise])
      })
      .spread(function (rawChunk, prevHeader) {
...
Verified.prototype.getHeader = function (id) {
      ...
      .then(function (header) {
        return self.getHeader(header.height)
          .then(function (localHeader) {
            ...
            throw new errors.Blockchain.VerifyHeaderError(id, 'not in local blockchain')
          })
      })
  }
  ...
  return Promise.try(function () {
    if (height > self.currentHeight) {
      throw new errors.Blockchain.VerifyHeaderError(height, 'not in local blockchain')
    }
