 ...
 * @param {number} [opts.concurrency=0] Zero is unlimited
 */
function Connector (opts) {
  EventEmitter.call(this)

  opts = _.extend({networkName: 'livenet', concurrency: 10}, opts)
...
Connector.prototype.getCurrentActiveRequests = function () {
  var msg = this.constructor.name + '.getCurrentActiveRequests'
  throw new errors.NotImplemented(msg)
}
...
Connector.prototype.getTimeFromLastResponse = function () {
  var msg = this.constructor.name + '.getTimeFromLastResponse'
  throw new errors.NotImplemented(msg)
}
...
Connector.prototype.headersQuery = function () {
  return Promise.reject(
    new errors.NotImplemented(this.constructor.name + '.headersQuery'))
}
...
Connector.prototype.getTxMerkle = function () {
  return Promise.reject(
    new errors.NotImplemented(this.constructor.name + '.getTxMerkle'))
}
...
Connector.prototype.subscribe = function () {
  return Promise.reject(
    new errors.NotImplemented(this.constructor.name + '.subscribe'))
}
