        ...
        return Promise.try(function () {
          ...
          switch (body.status) {
            case 'success':
              return body.data
            case 'fail':
              var err = new errors.Connector.Chromanode.Fail(body.data.type)
              err.data = body.data
              throw err
            case 'error':
              throw new errors.Connector.Chromanode.Error(body.message)
            default:
              var msg = 'Unknow status -- ' + body.status
              throw new errors.Connector.Chromanode.Error(msg)
          }
        })
        .finally(function () {
...
Chromanode.prototype.subscribe = util.makeSerial(function (opts) {
  ...
  return Promise.try(function () {
    ...
    if (request.event === 'newBlock') {
      self._socket.on('block', util.makeSerial(function (hash, height) {
