 ...
 * @return {Promise<ChromaInsight~HeaderObject>}
 */
...
/**
 * @param {string} txId
 * @return {Promise<ChromaInsight~TxBlockHashObject>}
 */
 ...
 * @return {Promise<ChromaInsight~UnspentObject[]>}
 */
ChromaInsight.prototype.getUnspent = function (address) {
...
ChromaInsight.prototype.subscribe = util.makeSerial(function (opts) {
  ...
  return Q.fcall(function () {
    ...
    if (request.event === 'touchAddress') {
      self._socket.on(request.address, function (txInfo) {
        self.emit('touchAddress', request.address, txInfo.txId)
      })
