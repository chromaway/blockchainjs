 ...
 * @param {boolean} [opts.spv=false] Value of supportsSPV method
 */
function Switcher (networks, opts) {
  ...
  self._networkHeights = self.networks.map(function () { return -1 })
  ...
  self.networks.forEach(function (network) {
    network.on('error', function (err) { self.emit('error', err) })
  })
  ...
  self.networks.forEach(function (network, index) {
    ...
    function onNewBlock (blockHash, height) {
      self._networkHeights[index] = height
    ...
    if (network.isConnected()) {
      network.getHeader('latest')
        .then(function (header) { onNewBlock(header.hash, header.height) })
    }
  })
  ...
  function newBlockHandler (blockHash, height) {
...
Switcher.prototype._updateCurrentNetwork = util.makeSerial(function () {
  ...
  var network = _.chain(self.networks)
    ...
    .filter(function (network) {
      ...
      return !self.supportsSPV() || network.supportsSPV()
    })
 ...
 * @return {boolean}
 */
Switcher.prototype.supportsSPV = function () {
  return this._supportsSPV
}
...
Switcher.prototype.getTxBlockHash = function () {
  ...
  return self._callMethod('getTxBlockHash', _.slice(arguments))
    .then(function (response) {
      if (!self.supportsSPV() && response.data !== null) {
 ...
 * @return {string}
 */
Switcher.prototype.inspect = function () {
  return '<network.Switcher for ' + this.networkName + ' network>'
}
